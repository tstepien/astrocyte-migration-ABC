function [c1_new,c2_new] = cellpops_separate(j,c1_old,c2_old,k_new,q1,q2,...
    PO2,dt,r,Pm,kappa,mu,alpha10,alpha11,alpha12,alpha13,alpha20,alpha21,...
    alpha22,alpha23,beta0,beta1,beta2,beta3,beta4,eta1,eta2,ce,cmax,hyaloid)
% [c1_new,c2_new] = cellpops_separate(j,c1_old,c2_old,k_new,q1,q2,...
%     PO2,dt,r,Pm,kappa,mu,alpha10,alpha11,alpha12,alpha13,alpha20,alpha21,...
%     alpha22,alpha23,beta0,beta1,beta2,beta3,beta4,eta1,eta2,ce,cmax,hyaloid)
%
...
g1 = growthterms_APC(c1_old(1:j),c2_old(1:j),q1(1:j),q2(1:j),PO2(1:j),...
    Pm,alpha10,alpha11,alpha12,alpha13,beta0,beta1,beta2,beta3,beta4,...
    eta1,cmax,hyaloid);
g2 = growthterms_IPA(c1_old(1:j),c2_old(1:j),q1(1:j),q2(1:j),PO2(1:j),...
    Pm,alpha20,alpha21,alpha22,alpha23,beta0,beta1,beta2,beta3,beta4,...
    eta2,cmax,hyaloid);
...
% 
% -1/(mu*r(j)*dr^2) * ...
% ( rhalf(j)*(c1_new(j)+c1_new(j+1))/2*Tderivative((k_new(j)+k_new(j+1))/2,kappa)*(k_new(j+1)-k_new(j)) ...
% - rhalf(j-1)*(c1_new(j-1)+c1_new(j))/2*Tderivative((k_new(j-1)+k_new(j))/2,kappa)*(k_new(j)-k_new(j-1)) ) ...
% +g11(j)*c1_new(j)
% 
