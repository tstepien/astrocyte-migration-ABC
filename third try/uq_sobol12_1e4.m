...
InputOpts.Marginals(1).Parameters = [0.0001 0.1];  % (mN h/mm^3)
...
InputOpts.Marginals(5).Name = '$\alpha_{21}$';  % proliferation rate IPA wrt PDGFA
InputOpts.Marginals(5).Type = 'Uniform';
InputOpts.Marginals(5).Parameters = [0 2];  % (/hr)
InputOpts.Marginals(5).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(6).Name = '$\alpha_{22}$';  % proliferation rate IPA wrt choroid oxygen
InputOpts.Marginals(6).Type = 'Gaussian';
InputOpts.Marginals(6).Parameters = [bestfitdist_param{6}.mu bestfitdist_param{6}.sigma];  % (/hr)
InputOpts.Marginals(6).Bounds = [0 1];  % (/hr)

InputOpts.Marginals(7).Name = '$\beta_0$';  % base differentiation rate
InputOpts.Marginals(7).Type = 'Weibull';
InputOpts.Marginals(7).Parameters = [bestfitdist_param{7}.A bestfitdist_param{7}.B];  % (/hr)
InputOpts.Marginals(7).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(8).Name = '$\beta_1$';  % differentiation rate wrt LIF
InputOpts.Marginals(8).Type = 'Uniform';
InputOpts.Marginals(8).Parameters = [0 2];  % (/hr)
InputOpts.Marginals(8).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(9).Name = '$\beta_2$';  % differentiation rate wrt choroid oxygen
InputOpts.Marginals(9).Type = 'Weibull';
InputOpts.Marginals(9).Parameters = [bestfitdist_param{7}.A bestfitdist_param{7}.B];  % (/hr)
InputOpts.Marginals(9).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(10).Name = '$\beta_4$';  % mass action rate
InputOpts.Marginals(10).Type = 'Uniform';
InputOpts.Marginals(10).Parameters = [0 2];  % (/hr)
InputOpts.Marginals(10).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(11).Name = '$\eta_1$';  % apoptosis rate APC
InputOpts.Marginals(11).Type = 'Weibull';
InputOpts.Marginals(11).Parameters = [bestfitdist_param{11}.A bestfitdist_param{11}.B];  % (/hr)
InputOpts.Marginals(11).Bounds = [0 2];  % (/hr)

InputOpts.Marginals(12).Name = '$\eta_2$';  % apoptosis rate IPA
InputOpts.Marginals(12).Type = 'Gamma';
