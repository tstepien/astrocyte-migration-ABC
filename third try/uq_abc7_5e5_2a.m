...
% because P_hy=0
% set equal to 0: beta0, eta1, eta2, alpha10, alpha22
...
alpha21 = (bound(4,2) - bound(4,1))*LHpts(:,4) + bound(4,1);
beta1   = (bound(5,2) - bound(5,1))*LHpts(:,5) + bound(5,1);
beta2   = (bound(6,2) - bound(6,1))*LHpts(:,6) + bound(6,1);
beta4   = (bound(7,2) - bound(7,1))*LHpts(:,7) + bound(7,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% run simulations %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
parfor i=1+1e5:1.5e5
    disp(['iteration i: ',num2str(i)])
    %%% solve equation
    [t,r,c1,c2,~,~,mvgbdy,~,~] = eqnsolver(mu(i),0,alpha11(i),...
        alpha12(i),0,0,alpha21(i),0,0,...
        0,beta1(i),beta2(i),0,beta4(i),0,0,...
        0,1,m);
