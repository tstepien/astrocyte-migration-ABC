...
load(strcat(pwd,'/LHpts/LHpts',num2str(num_param),'uni_',num2str(multiplier),...
    'e',num2str(power),'.mat'))
...
%     if strcmp(doublecheck,'y')==1
        diary(strcat(pwd,'/parameter_analysis/diary_abc',num2str(num_param),'uni_',...
...
alpha13 = 0;
alpha20 = (bound(4,2) - bound(4,1))*LHpts(:,4) + bound(4,1);
...
parfor i=1+2e5:3e5
    ...
    [t,r,c1,c2,~,~,mvgbdy,~,~] = eqnsolver(mu(i),alpha10(i),alpha11,...
        ...
        beta0,beta1(i),beta2(i),beta3,beta4,eta1,eta2(i),...
        P_hy,r_hy,m,oxyfunc);
...
% if strcmp(savefiles,'yes')==1
    save(strcat(pwd,'/parameter_analysis/abc',num2str(num_param),'uni_',...
