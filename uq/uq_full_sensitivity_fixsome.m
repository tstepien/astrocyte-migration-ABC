...
ModelOpts.mFile = 'uq_eqns_and_error_fixsome';
...
InputOpts.Marginals(8).Name = '$P_\mathrm{hy}$';  % partial pressure of oxygen due to hyaloid artery
InputOpts.Marginals(8).Type = 'Uniform';
InputOpts.Marginals(8).Parameters = [0 10];  % (dimensionless)
InputOpts.Marginals(8).Bounds = [0 10];  % (dimensionless)

InputOpts.Marginals(9).Name = '$r_\mathrm{hy}$';  % radius at half-maximum of Hill function for hyaloid
InputOpts.Marginals(9).Type = 'Uniform';
InputOpts.Marginals(9).Parameters = [0.001 1];  % (mm)
InputOpts.Marginals(9).Bounds = [0.001 1];  % (mm)
