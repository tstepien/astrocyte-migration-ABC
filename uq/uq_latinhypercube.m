...
bound = [0.01 2; %mu - adhesion constant
    0 0.1; %alpha10 - (/hr) base proliferation rate APC
    0 0.1; %alpha11 - (/hr) proliferation rate APC wrt oxygen
    0 0.1; %alpha12 - (/hr) proliferation rate APC wrt PDGFA
    0 0.1; %alpha20 - (/hr) base proliferation rate IPA
    0 0.1; %alpha21 - (/hr) proliferation rate IPA wrt oxygen
    0 0.1; %alpha22 - (/hr) proliferation rate IPA wrt PDGFA
    0 0.1; %beta0 - (/hr) base differentiation rate
    0 0.1; %beta1 - (/hr) mass action rate
    0 0.1; %beta2 -  (/hr) differentiation rate wrt oxygen
    0 0.1; %beta3 -  (/hr) differentiation rate wrt LIF
...
beta2   = (bound(10,2) - bound(10,1))*LHpts(:,10) + bound(10,1);
beta3   = (bound(11,2) - bound(11,1))*LHpts(:,11) + bound(11,1);
eta1    = (bound(12,2) - bound(12,1))*LHpts(:,12) + bound(12,1);
eta2    = (bound(13,2) - bound(13,1))*LHpts(:,13) + bound(13,1);
P_hy    = (bound(14,2) - bound(14,1))*LHpts(:,14) + bound(14,1);
r_hy    = (bound(15,2) - bound(15,1))*LHpts(:,15) + bound(15,1);
...
parfor i=1:N
    ...
    [t,r,c1,c2,~,~,mvgbdy,~,~] = eqnsolver(mu(i),alpha10(i),alpha11(i),...
        alpha12(i),alpha20(i),alpha21(i),alpha22(i),beta0(i),beta1(i),...
        beta2(i),beta3(i),eta1(i),eta2(i),P_hy(i),r_hy(i),m);
