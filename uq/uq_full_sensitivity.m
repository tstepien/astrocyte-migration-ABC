...
InputOpts.Marginals(2).Parameters = [0.6881 0.2105];  % (/hr)
...
InputOpts.Marginals(4).Name = '$\alpha_{21}$';  % proliferation rate IPA wrt oxygen
InputOpts.Marginals(4).Type = 'Exponential';
InputOpts.Marginals(4).Parameters = 0.3481;  % (/hr)
InputOpts.Marginals(4).Bounds = [0 1];  % (/hr)

InputOpts.Marginals(5).Name = '$\alpha_{22}$';  % proliferation rate IPA wrt PDGFA
InputOpts.Marginals(5).Type = 'Uniform';
InputOpts.Marginals(5).Parameters = [0 1];  % (/hr)
InputOpts.Marginals(5).Bounds = [0 1];  % (/hr)

InputOpts.Marginals(6).Name = '$\beta_1$';  % mass action rate
InputOpts.Marginals(6).Type = 'Uniform';
InputOpts.Marginals(6).Parameters = [0 1];  % (/hr)
InputOpts.Marginals(6).Bounds = [0 1];  % (/hr)

InputOpts.Marginals(7).Name = '$\beta_2$';  % differentiation rate wrt O2
InputOpts.Marginals(7).Type = 'Exponential';
InputOpts.Marginals(7).Parameters = 0.1216;  % (/hr)
InputOpts.Marginals(7).Bounds = [0 1];  % (/hr)

InputOpts.Marginals(8).Name = '$\beta_3$';  % differentiation rate wrt LIF
...
InputOpts.Marginals(11).Name = '$T_e$';  % tension on boundary
InputOpts.Marginals(11).Type = 'Gaussian';
InputOpts.Marginals(11).Parameters = [0.0028 0.0007];  % (N/mm)
InputOpts.Marginals(11).Bounds = [0.0001 0.0038];  % (N/mm)

InputOpts.Marginals(12).Name = '$P_\mathrm{hy}$';  % partial pressure of oxygen due to hyaloid artery
InputOpts.Marginals(12).Type = 'Gaussian';
InputOpts.Marginals(12).Parameters = [8.2809 5.0386];  % (dimensionless)
InputOpts.Marginals(12).Bounds = [0 20];  % (dimensionless)

InputOpts.Marginals(13).Name = '$r_\mathrm{hy}$';  % radius at half-maximum of Hill function for hyaloid
InputOpts.Marginals(13).Type = 'Gaussian';
InputOpts.Marginals(13).Parameters = [0.5456 0.2394];  % (mm)
InputOpts.Marginals(13).Bounds = [0.001 1];  % (mm)
